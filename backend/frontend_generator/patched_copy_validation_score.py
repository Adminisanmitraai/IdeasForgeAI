...
def _locked_flags() -> dict[str, Any]:
    return {
        ...
        "section_patch_applied_by_this_phase": False,
...
def get_phase17f_patched_copy_validation_score(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    ...
    if not PHASE17_TARGET.exists():
        errors.append("Phase 17 target folder is missing")
    ...
    required_files_ok = PHASE17_TARGET.exists() and not missing_files and not extra_files
    ...
    if not missing_files:
        index_html = _read_text(PHASE17_TARGET / "index.html")
        styles_css = _read_text(PHASE17_TARGET / "styles.css")
        app_js = _read_text(PHASE17_TARGET / "app.js")

        manifest_json, manifest_errors = _read_json(PHASE17_TARGET / "manifest.json")
        rollback_manifest, rollback_errors = _read_json(PHASE17_TARGET / "rollback-manifest.json")
    ...
    score_categories["html_patch_marker_score"] = _score(html_patch_marker_ok)
    ...
    rollback_manifest_ok = bool(
        ...
        and rollback_manifest.get("patched_file_hashes", {}).get("index.html")
    )
    score_categories["rollback_manifest_score"] = _score(rollback_manifest_ok)

    phase17_report = ""
    if (PHASE17_TARGET / "phase17-validation-report.md").exists():
        phase17_report = _read_text(PHASE17_TARGET / "phase17-validation-report.md").lower()

    phase17_report_ok = (
        "status: success" in phase17_report
        and "patch applied to copied html only: true" in phase17_report
        and "real generated app modified: false" in phase17_report
        and "deployment unlocked: false" in phase17_report
    )
    score_categories["phase17_report_score"] = _score(phase17_report_ok)

    application_report = ""
    if (PHASE17_TARGET / "section-patch-application-report.md").exists():
        application_report = _read_text(PHASE17_TARGET / "section-patch-application-report.md").lower()

    patch_application_report_ok = (
        "status: success" in application_report
        and "patch applied only to phase 17 sandbox copy" in application_report
        and "real generated app modified: false" in application_report
        and "generated-apps/ideasforgeai-preview-v1 touched: false" in application_report
    )
    score_categories["patch_application_report_score"] = _score(patch_application_report_ok)
    ...
    app_visible_combined = "\n".join(
        _read_text(PHASE17_TARGET / name)
    ...
    return {
        ...
        "phase": "Phase 17F - Patched Copy Validation Score",
