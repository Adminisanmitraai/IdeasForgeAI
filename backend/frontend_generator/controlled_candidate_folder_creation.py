...
APPROVED_CANDIDATE_FILES = REQUIRED_SOURCE_FILES + [
...
def _locked_flags() -> dict[str, Any]:
    return {
        "controlled_candidate_folder_creation_only": True,
        "candidate_creation_performed": True,
        "candidate_manifest_created": True,
        "candidate_folder_created": True,
        "approved_phase19_folder_write_only": True,
        "file_write_allowed_outside_phase19_folder": False,
...
def _candidate_id_valid(value: str) -> bool:
    return bool(re.fullmatch(r"IF-CANDIDATE-19E-[A-Za-z0-9-]{4,64}", value or ""))
...
def _validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("source_phase") != "Phase 19E":
        errors.append("source_phase must equal Phase 19E")

    if not _candidate_id_valid(str(payload.get("candidate_id", ""))):
        errors.append("candidate_id must match IF-CANDIDATE-19E-* format")
    ...
    if payload.get("phase19d_dry_run_validation_passed") is not True:
        errors.append("phase19d_dry_run_validation_passed must be true")
...
def _validate_source_files() -> tuple[list[str], list[str], list[str]]:
    ...
    if not SOURCE_FOLDER.exists():
        errors.append("approved Phase 18 source folder does not exist")
    ...
    if not (SOURCE_FOLDER / "promotion-manifest.json").exists():
        errors.append("promotion-manifest.json is required in Phase 18 source")

    if not (SOURCE_FOLDER / "rollback-manifest.json").exists():
        errors.append("rollback-manifest.json is required in Phase 18 source")

    return errors, warnings, existing_files


def create_phase19e_controlled_candidate_folder(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    ...
    if errors:
        ...
        locked["candidate_creation_performed"] = False
        locked["candidate_manifest_created"] = False
        locked["candidate_folder_created"] = False
        return {
            "status": "blocked",
            "phase": "Phase 19E - Controlled Candidate Folder Creation",
    ...
    for item in TARGET_FOLDER.iterdir():
        if item.is_dir():
            raise RuntimeError(f"unexpected directory in Phase 19 target: {item.name}")
        if item.is_file():
            item.unlink()
    ...
    for file_name in REQUIRED_SOURCE_FILES:
        ...
        candidate_hashes[file_name] = _sha256(target_file)
        candidate_files.append(str(target_file))

    now = datetime.now(timezone.utc).isoformat()

    candidate_manifest = {
        "candidate_manifest_version": "19E.1",
        "phase": "Phase 19E - Controlled Candidate Folder Creation",
        "created_at": now,
        ...
        "source_folder": str(SOURCE_FOLDER),
        "target_folder": str(TARGET_FOLDER),
        "source_validation_score": int(payload.get("phase18g_validation_score", 0)),
        "phase18g_validation_passed": True,
        "phase18h_frozen": True,
        "phase19c_approval_validated": True,
        "phase19d_dry_run_validation_passed": True,
        "copied_files": REQUIRED_SOURCE_FILES,
        "source_file_hashes": source_hashes,
        "candidate_file_hashes": candidate_hashes,
        "promotion_manifest_source": str(SOURCE_FOLDER / "promotion-manifest.json"),
        "rollback_manifest_source": str(SOURCE_FOLDER / "rollback-manifest.json"),
        "rollback_available": True,
        "candidate_preview_route": "/api/frontend-generator/phase19f-main-preview-candidate/index.html",
        "candidate_output_validation_required": True,
        "production_replacement_allowed": False,
...
Candidate creation performed: true
Candidate target: {TARGET_FOLDER}
Candidate source: {SOURCE_FOLDER}
...
"""
    ...
    validation_report = f"""# Phase 19E Validation Report
...
"""
    ...
    control_outputs = {
        "candidate-manifest.json": json.dumps(candidate_manifest, indent=2),
        "phase19-candidate-report.md": candidate_report,
        "phase19-validation-report.md": validation_report,
    }
    ...
    missing_target_files = [name for name in APPROVED_CANDIDATE_FILES if name not in existing_target_files]
    extra_target_files = [name for name in existing_target_files if name not in APPROVED_CANDIDATE_FILES]
    ...
    return {
        ...
        "validation_errors": [] if validation_passed else ["Phase 19 candidate file set mismatch"],
        "validation_warnings": warnings,
        "source_folder": str(SOURCE_FOLDER),
        "target_folder": str(TARGET_FOLDER),
