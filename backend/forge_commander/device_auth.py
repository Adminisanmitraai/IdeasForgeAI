...
FORGE_COMMANDER_DEVICE_AUTH_VERSION = "forge-commander.device-auth.v2"
...
def _device_signing_key(signing_key: str) -> str:
    epoch = os.getenv("FORGE_COMMANDER_DEVICE_SIGNING_EPOCH", "2").strip() or "2"
    derived = hmac.new(signing_key.encode("utf-8"), f"forge-device-auth:{epoch}".encode("utf-8"), sha256).hexdigest()
    return derived
...
def issue_device_token(owner_subject: str, device_id: str, *, signing_key: str, expires_at: int) -> str:
    ...
    device_key = _device_signing_key(signing_key)
...
def parse_device_token(token: str, *, expected_device_id: str, signing_key: str | None = None,
    ...
    device_key = _device_signing_key(key)
    expected = _b64url(hmac.new(device_key.encode("utf-8"), payload.encode("ascii"), sha256).digest())
