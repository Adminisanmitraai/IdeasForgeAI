...
# Corrected visual centers from the actual reference image.
ICON_CENTERS: Dict[str, Tuple[int, int]] = {
    "header_logo": (166, 212),
    "forge_studio": (134, 718),
    "forge_code": (134, 975),
    "forge_work": (134, 1166),
}

SIZES = [96, 104, 112, 120, 128]
OFFSETS = [
    (0, 0),
    (-6, 0),
    (6, 0),
...
def crop_square(img: Image.Image, cx: int, cy: int, size: int):
    half = size // 2
    x1 = max(0, cx - half)
    y1 = max(0, cy - half)
...
def main() -> int:
    ...
    run_dir = OUT_DIR / f"reference-icon-candidates-focused-{stamp}"
    ...
    for icon_name, center in ICON_CENTERS.items():
        ...
        for size in SIZES:
            for ox, oy in OFFSETS:
                ...
                candidate_name = f"{icon_name}_f{index:02d}_s{size}_ox{ox}_oy{oy}"
        ...
        sheet_path = icon_dir / f"{icon_name}_focused_candidate_sheet.png"
    ...
    report = {
        "ok": True,
        "phase": "PIXEL-MAP-PERFECT-16F",
        "created_at": stamp,
        "reference_path": str(reference_path),
        "output_dir": str(run_dir),
        "assets": report_assets,
        "rule": "Focused candidate crops only. Do not apply to frontend yet.",
        "next_step": "Open sheets and choose the cleanest full icon tile for each icon.",
    }

    report_path = run_dir / "focused-icon-candidate-report.json"
