...
function forwardToDesktop(ws, msg, label) {
  ...
  broadcast({ type: "LOG", message: `${label} sent: ${msg.action || msg.command || msg.type}` });
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "MOUSE_COMMAND") {
      const allowed = ["MOVE", "LEFT_CLICK", "RIGHT_CLICK", "DOUBLE_CLICK"];
      if (!allowed.includes(msg.action)) {
        send(ws, { type: "LOG", message: "Mouse command blocked: invalid action." });
        return;
      }
      forwardToDesktop(ws, msg, "Mouse command");
      return;
    }

    if (msg.type === "KEYBOARD_COMMAND") {
      const allowed = ["TYPE_TEXT", "KEY", "HOTKEY"];
      if (!allowed.includes(msg.action)) {
        send(ws, { type: "LOG", message: "Keyboard command blocked: invalid action." });
        return;
      }
      forwardToDesktop(ws, msg, "Keyboard command");
      return;
    }

    if (msg.type === "START_SCREEN_STREAM") {
      forwardToDesktop(ws, msg, "Screen stream start");
      return;
    }

    if (msg.type === "STOP_SCREEN_STREAM") {
      forwardToDesktop(ws, msg, "Screen stream stop");
      return;
    }
    ...
    if (msg.type === "SCREEN_STATUS") {
      if (ws.role !== "desktop") return;
      broadcast({ type: "LOG", message: `Screen stream ${msg.status}.` });
      return;
    }

    if (msg.type === "SCREEN_ERROR") {
      if (ws.role !== "desktop") return;
      broadcast({ type: "LOG", message: `Screen error: ${msg.message}` });
      return;
    }

    if (msg.type === "MOUSE_RESULT" || msg.type === "KEYBOARD_RESULT") {
      if (ws.role !== "desktop") return;
      broadcast({ type: "LOG", message: msg.message || "Command completed." });
      return;
    }
  });
...
server.listen(PORT, () => {
  console.log(`ForgePilot Cloud Gateway FP-3C-2 running on http://localhost:${PORT}`);
});
