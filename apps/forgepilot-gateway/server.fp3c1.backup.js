...
function forwardToDesktop(ws, msg, commandType) {
  if (ws.role !== "browser") return;

  if (!trusted) {
    send(ws, { type: "LOG", message: `${commandType} blocked: desktop is not trusted.` });
    return;
  }

  if (!desktopSocket || desktopSocket.readyState !== WebSocket.OPEN) {
    send(ws, { type: "LOG", message: `${commandType} blocked: desktop offline.` });
    return;
  }

  send(desktopSocket, msg);
  broadcast({ type: "LOG", message: `${commandType} sent: ${msg.action || msg.command || ""}` });
}
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "REGISTER_DESKTOP_COMPANION") {
      ...
      desktop = {
        ...
        mode: msg.mode || "mouse + keyboard control",
        version: msg.version || "FP-3B",
    ...
    if (msg.type === "KEYBOARD_COMMAND") {
      const allowedActions = ["TYPE_TEXT", "KEY", "HOTKEY"];
      if (!allowedActions.includes(msg.action)) {
...
server.listen(PORT, () => {
  console.log(`ForgePilot Cloud Gateway FP-3B running on http://localhost:${PORT}`);
});
